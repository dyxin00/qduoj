...
int execute_cmd(const char * fmt, ...) {   //system shell
...
void _update_solution_mysql(int solution_id, int result, int time, int memory,double pass_rate) {
...
* */
void update_solution(int solution_id, int result, int time, int memory,double pass_rate) {
	if(result==OJ_TL&&memory==0) 
		result=OJ_ML;
	
	_update_solution_mysql( solution_id,  result,  time,  memory,pass_rate);
...
void run_solution(int & lang, char * work_dir, int & time_lmt, int & usedtime,
	...
	// trace me
	ptrace(PTRACE_TRACEME, 0, NULL, NULL);  //进程跟踪
	// run me
	if (lang != 3)
		chroot(work_dir);  //更改根目录

	while(setgid(1536)!=0) sleep(1);
	while(setuid(1536)!=0) sleep(1);  
	while(setresuid(1536, 1536, 1536)!=0) sleep(1);  //用户处理
...
int main(int argc, char** argv) {
	...
	char work_dir[BUFFER_SIZE];   //运行目录
	//char cmd[BUFFER_SIZE];
	...
	if(shm_run) mk_shm_workdir(work_dir);  //是否使用/dev/shm的共享内存虚拟磁盘来运行答案，如果启用能提高判题速度，但需要较多内存。  

	chdir(work_dir); //改变当前工作目录  /home/judge/run1
	if (!DEBUG)
	...
	get_solution(solution_id, work_dir, lang);  //获取源码 保存在 Main  wrok_dir 中
	...
	sprintf(fullpath, "%s/data/%d", oj_home, p_id); // the fullpath of data dir 数据位置


	// open DIRs  //打开一个目录，在失败的时候返回一个空的指针。
	DIR *dp;
	dirent *dirp;
 ...
 */
	...
	*/
	...
	if(oi_mode){
		...
		update_solution(solution_id, finalACflg, usedtime, topmemory >> 10, pass_rate);
	}else{
